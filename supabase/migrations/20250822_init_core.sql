...
create table if not exists public.inspection_items (
  ...
  value text, notes text, require_photo boolean default false
);
...
create table if not exists public.photos (
  id uuid primary key default gen_random_uuid(),
  inspection_id uuid not null references public.inspections(id) on delete cascade,
  item_path text, file_url text not null, thumb_url text, exif_json jsonb,
  created_at timestamptz default now()
);
...
create table if not exists public.signatures (
  ...
  signed_by_name text, signed_at timestamptz, ip text, geo_json jsonb, file_url text not null
);
...
create table if not exists public.pdfs (
  ...
  storage_path text,            -- caminho no bucket (ex: org/inspect/file.pdf)
...
create table if not exists public.audit_logs (
  ...
  action text, entity text, entity_id uuid, metadata jsonb,
  created_at timestamptz default now()
);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname='public' AND tablename='organizations' AND policyname='orgs_select_owner'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='profiles' AND policyname='profiles_same_org_select'
  ) THEN
